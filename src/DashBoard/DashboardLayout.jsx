...
import { RiDashboardLine, RiTaskLine, RiCalendarLine, RiBarChartLine, RiTeamLine, RiSettingsLine, RiQuestionLine, RiLogoutBoxLine, RiSmartphoneLine, RiCloseLine} from "react-icons/ri";
...
export default function SidebarNavbar() {
  ...
  const SidebarContent = () => (
    <aside className="w-52 bg-white flex flex-col justify-between py-6 px-4 shadow-sm h-full">
      <div>
        {/* Logo */}
        ...
        <ul className="space-y-0.5">
          {menuItems.map((item) => (
            <li key={item.label}>
              <button
                onClick={() => { setActive(item.label); closeSidebar(); }}
              ...
              >
                ...
                {item.badge && (
                  <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                    active === item.label ? "bg-green-600 text-green-100" : "bg-green-100 text-green-700"
                  }`}>
                    {item.badge}
                  </span>
                )}
    ...
    <div className="flex min-h-screen bg-gray-100 font-sans">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 flex lg:hidden">
          ...
          <button
            onClick={closeSidebar}
            className="absolute top-4 left-[13.5rem] z-50 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md"
          >
            <RiCloseLine className="text-gray-600 text-lg" />
          </button>
        </div>
      )}

      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <DBNavBar onMenuClick={() => setSidebarOpen(true)} />

        <div className="flex-1 p-4 sm:p-6 text-black overflow-auto">
          <Outlet />
        </div>
      </div>
    </div>
